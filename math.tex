\begin{enumerate}
    ...
    Посчитаем пары взаимно-простых в $[1, n]$: $\sum_{i,j=1}^{n}[gcd(i, j)=1] = \sum_{i,j=1}^{n}\sum_{d|gcd(i, j)}\mu(d)= \sum_{i,j=1}^{n}\sum_{d=1}^{n}[d|gcd(i, j)]\mu(d) = \sum_{i,j=1}^{n}\sum_{d=1}^{n} [d|i][d|j]\mu(d) = \sum_{d=1}^{n}\mu(d) (\lfloor \frac{n}{d} \rfloor)^2$
    ...
    \[
        \left[ {n \atop k} \right] = \left[ {n-1 \atop k-1} \right] + (n-1) \cdot \left[ {n-1 \atop k} \right], \quad 
        \left[ {0 \atop 0} \right] = 1 \quad
        \left[ {n \atop 0} \right] = 0 \quad
    ...
    \item \textbf{Числа Эйлера (первого рода).} Количество перестановок порядка $n$ с $k$ подъемами
    \[
    ...
    \item \textbf{Функция Эйлера} \\
    $a^{\phi(m)} \equiv 1 (mod \, m) \quad a^{n} \equiv a^{n \, mod \, \phi(m)} (mod \, m) \quad \sum_{d|n} \phi(d) = n$
