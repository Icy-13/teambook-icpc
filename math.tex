\begin{enumerate}
    ...
    $C_n = C_{n-1} \cdot \frac{4n - 2}{n+1}, C_0 = 1$
    ...
    \item \textbf{Цепи Маркова}
    До поглощения (куда поглотимся? матожидание шагов до поглощения?):

    Переномируем состояния, чтобы сначала шли непоглощающие, затем поглощающие. Ниже считаем, что поглощающие классы состоят из 1 вершины:  
        \[
    \left[
    \begin{array}{c|c}
        Q & R \\
    ...
    $N = (I - Q)^{-1}$, $a_0$ - начальное.

    \begin{enumerate}
        \item Матожидание количества посещений $i$-той непоглощающей: $ET_i = a_0 N$
        \item Матожидание шагов до поглощения: $ET = a_0 N \vec{1}$
        \item Вероятности поглощения: $A = a_0 N R$
    \end{enumerate}

    После поглощения (предельное распределение):
    
    Теперь живем внутри поглощающего класса. Если НОД циклов $>1$, класс периодический. В нем НЕТ предельного распределения.
    Иначе: $a_0 P \rightarrow b$.
    
    Как найти b? Решить систему 
    $(I - P)b = 0$ и $\sum_i b_i = 1$ или же просто возвести $P$ в квадрат много раз.

    В пределе внутри $i$ столбца $P^n$ стоит какое-то число $a_i$.
    ...
    \begin{enumerate}
        \item \%=, = на отрезке, get ;; lognlogC ;; break: maxvalue[node] < mod ;; tag: maxvalue[node]==minvalue[node]
        \item min=, max=, += $\sum$, min, max, (gcd) ;; logn ;; break: max <= x ;; tag: secondMax < x
        \item $\sqrt{}$=, /=, +=, sum, min, max ;; lognlogC ;; break: default ;; tag: $max[v] - min[v] \le 1$ 
        \item \&=, |=, max ;; lognlogC ;; break: default ;; tagAnd: ((and[v] $\oplus$ or[v]) \& $\sim$ x)==0 ;; tagOr: ((and[v] $\oplus$ or[v]) \& x)==0
    \end{enumerate}
    ...
    \item \textbf{Парсочи} \begin{enumerate}
        \item Диниц работает на двудольных графах (пропускная способность каждого ребра = 1, добавить сток и исток) за $O(m \sqrt{n})$
        \item Макс парсоч мин веса (вес на вершинах) - отсортировать вершины левой доли по весу и Кун. Если вес на ребрах, то лучше это поток минимальной стоимости.
        \item Покрытие путями DAG: двудольный граф по $n$ в каждой доле, $(i, j) \in G \implies (a_i, b_j) \in H$. Находим макс парсоч в $H$, восстановление путей: $a_1 \to b_k, a_k \to ...$.
        \item Минимальное вершинное покрытие в двудольном графе равно размеру макс парсоча. Построим макс парсоч, ребра из него ориентируем справа налево. Запустим дфс из всех вершин левой доли не в $M$. Тогда $V_{min} = L^{-} \cup R^{+}$
        \item Максимальное независимое множество является дополнением минимального вершинного покрытия
        \item $R^+$ всегда содержится в минимальном покрывающем.
    \end{enumerate}
    ...
    \item {Арифметическая прогрессия} $S_n = \frac{2a_1 + (n - 1) d}{2} \cdot n $
    \item {Геометрическая прогрессия} $S_n = \frac{b_1 (q^n - 1)}{q - 1}$
    \item \textbf{Матожидание}
    \begin{enumerate}
        \item Если $\xi$ неотрицательная целочисленная $E \xi = \sum_{i=0}^\infty P(\xi > i)$
        \item Формула полного матожидания $E X = E_Y E(X | Y)$
        \item Число неподвижных точек перестановки 1.
        \item Число инверсий перестановки $\frac{n(n-1)}{4}$
        \item Число циклов в перестановке $\ln n$
    \end{enumerate}
    \item {Монотонная последовательность} В последовательности длины $n$ найдется монотонная подпоследовательность длиной $\ge \sqrt n$ 
