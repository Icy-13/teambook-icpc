...
void printer_sample() {
    int x = 69;
    string s = "meow-princesses";
    print(x, s);
    std::pair<int, string> a = {42, "aboba"};
    print(a);
    std::vector<std::pair<std::vector<int>, std::string>> wow = {{{1, 2, 3}, "el1"}, {{4, 5, 6}, "el2"}};
    print(wow);
    std::vector<std::vector<int>> matrix(5, vector<int>(5));
    for (int i = 0; i < 5; i++) matrix[i][i] = 42;
    print(matrix);
}
